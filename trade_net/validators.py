...
class NameValidator:
    ...
    def __call__(self, value):
        ...
        if field_type == "Завод":
            if models.NetUnitModel.objects.filter(name=field_name).exists():
                raise ValidationError("Такой завод производитель уже существует!")
        if field_type == "Торговая сеть":
            if models.NetUnitModel.objects.filter(
                city=field_city, address=field_address
            ).exists():
                raise ValidationError(
                    "Магазин торговой сети по этому адресу уже существует!"
                )
...
class AddressValidator:
    ...
    def __call__(self, value):
        ...
        re_address = re.compile(
            r"^(ул\.|мкр\.)\s+[А-Яа-яЁё\s]+,\s*(д\.|корп\.|стр\.)\s*\d+$"
        )
        if field_address:
            if not re_address.match(field_address):
                raise ValidationError(
                    'Адрес должен иметь вид: ул./мкр."Название", д./корп./стр. "Номер"'
                )
